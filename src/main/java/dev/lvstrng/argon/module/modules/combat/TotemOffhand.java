...
public final class TotemOffhand extends Module implements TickListener {
    ...
    public TotemOffhand() {
        super(EncryptedString.of("Totem Offhand"),
                EncryptedString.of("Switches to your totem slot and offhands a totem if you dont have one already"),
                -1, Category.COMBAT);
    ...
    public void onTick() {
        ...
        if(active) {
            ...
            if (InventoryUtils.selectItemFromHotbar(Items.TOTEM_OF_UNDYING)) {
                ...
                if (!sent) {
                    Objects.requireNonNull(mc.getNetworkHandler()).getConnection().send(new PlayerActionC2SPacket(PlayerActionC2SPacket.Action.SWAP_ITEM_WITH_OFFHAND, BlockPos.ORIGIN, Direction.DOWN));
                    sent = true;
                    return;
                }
            }
